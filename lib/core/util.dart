...
List<PrizModel> calculatedItems() {
  Map<String, PrizModel> uniqueItems = {};

  for (var item in itemsInCart) {
    if (uniqueItems.containsKey(item.name)) {
      // uniqueItems[item.name]!.count += 1;
      uniqueItems[item.name] = uniqueItems[item.name]!.copyWith(count: uniqueItems[item.name]!.count + 1);
    } else {
      uniqueItems[item.name] = PrizModel(
        name: item.name,
        price: item.price,
        image: item.image,
        count: 1,
